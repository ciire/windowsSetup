...
function Invoke-AppRemoval {
    ...
    try {
        ...
        # [Existing Win32 uninstall logic preserved as-is]
...
function Install-FromCsv {
    ...
    foreach ($row in $rows) {
        ...
        if ($alreadyInstalled) {
            ...
            $results += [PSCustomObject]@{ Name = $name; Result = "Skipped"; ExitCode = $null }
            continue
        }

        if ($DryRun) {
            Write-InstallLog "  [DRY RUN] Would run: $path $silentArgs"
            $results += [PSCustomObject]@{ Name = $name; Result = "DryRun"; ExitCode = $null }
            continue
        }

        try {
            $exitCode = switch ($type) {
            ...
            $resultContext = switch ($result) {
                "Success"               { "exit code $exitCode - most likely a success" }
                "RebootRequired"        { "exit code $exitCode - most likely a success, but a reboot is required" }
                "Cancelled"             { "exit code $exitCode - most likely cancelled by the user" }
                "AnotherInstallRunning" { "exit code $exitCode - another install is already running" }
                default                 { "exit code $exitCode - most likely a failure" }
            }
            Write-InstallLog "  $name -> $result ($resultContext)"
            $results += [PSCustomObject]@{ Name = $name; Result = $result; ExitCode = $exitCode }
            ...
            $results += [PSCustomObject]@{ Name = $name; Result = "Failed"; ExitCode = $null }
        }
    }
    ...
    $results | ForEach-Object {
        $summaryContext = if ($null -ne $_.ExitCode) {
            switch ($_.Result) {
                "Success"               { "exit code $($_.ExitCode) - most likely a success" }
                "RebootRequired"        { "exit code $($_.ExitCode) - most likely a success, but a reboot is required" }
                "Cancelled"             { "exit code $($_.ExitCode) - most likely cancelled by the user" }
                "AnotherInstallRunning" { "exit code $($_.ExitCode) - another install was already running" }
                default                 { "exit code $($_.ExitCode) - most likely a failure" }
            }
        } else {
            $_.Result
        }
        Write-InstallLog "  $($_.Name): $summaryContext"
    }

    $succeeded = ($results | Where-Object Result -eq "Success").Count
    $cancelled = ($results | Where-Object Result -eq "Cancelled").Count
    $reboot    = ($results | Where-Object Result -eq "RebootRequired").Count
    $failed    = ($results | Where-Object { $_.Result -notin @("Success","RebootRequired","DryRun","Skipped","Cancelled") }).Count
